...
def load_audio_any(x: str) -> Tuple[np.ndarray, int]:
    ...
    try:
        if is_url(x):
            ...
            # Try soundfile first (faster, more reliable on Windows)
            try:
                audio, sr = sf.read(x, dtype="float32", always_2d=False)
            except Exception as sf_err:
                print(f"[load_audio] soundfile failed ({sf_err}), trying librosa...")
        ...
        print(f"[load_audio] FATAL: Failed to load audio: {x}")
        print(f"[load_audio] Error: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
        raise
